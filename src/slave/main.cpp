...
// =================== SLAVE (SN65HVD230 versiyonu) ====================
// TX_ID = 0x456 (yuksek ID -> arbitration'da gercek bir cakismada kaybetmesi beklenir)
// RX_ID = 0x123
...
void loop() {
  ...
  if (triggerFlag) {
    ...
    for (uint8_t burstIdx = 1; burstIdx <= BURST_COUNT; burstIdx++) {
      ...
      // ---- ARBITRATION YORUMU ----
      // SLAVE yuksek ID (0x456) tasiyor -> gercek bir cakismada kaybetmesi beklenir.
      if (arbLost) {
        mloaCountThisRound++;
        Serial.printf("[%s] >>> ARBITRATION KAYBEDILDI (beklenen davranis - MASTER/dusuk ID kazandi)\n", ROLE_NAME);
      } else if (res == ESP_OK) {
        Serial.printf("[%s] >>> Gonderim basarili (bu turda gercek bir cakisma olmadi - SLAVE ilk basladi ya da MASTER'dan yeterince ayrikti)\n", ROLE_NAME);
      } else {
